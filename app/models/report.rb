class Report
  ...
  def self.sprint_report_data(sprint)
    binding.pry
    @sprint_report_attributes = sprint&.issues.left_outer_joins(:time_logs)&.group('issues.assignee_id')&.select("issues.*, sum(time_logs.logged_time) as total_time_spent, count(*) AS total_issues, count('#{Issue::STATUS.invert['Resolved']}') as resolved_issues, sum(estimated_time) as total_estimated_time") if sprint.present?
  end
